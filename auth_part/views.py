...
def login(request):
    ...
    #if request.user:
    #    return render(request, 'userdashboard.html', context={'username': request.user.username})
...
def create_profile(request):
    ...
    if request.method == 'POST':
        ...
        if userform.is_valid() and (len(User.objects.filter(email=request.POST.get('email')))==0):
            ...
            return render(request, 'index.html', context={'sign_up': True, 'error': error, 'tried': True})
            
    
    userform = UserForm()
    return render(request, 'index.html', context={'sign_up': True, 'error': error})
